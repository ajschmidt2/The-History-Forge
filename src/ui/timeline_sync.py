...
def _apply_scene_media_assignments(timeline: Timeline, session_scenes: list[Any] | None, project_path: Path) -> None:
    ...
    session_by_index: dict[int, Any] = {}
    for scene in session_scenes:
        idx = getattr(scene, "index", None)
        if isinstance(idx, int) and idx > 0:
            session_by_index[idx] = scene

    for position, timeline_scene in enumerate(timeline.scenes, start=1):
        idx = _scene_index_from_stem(str(getattr(timeline_scene, "id", "") or ""), _scene_index_from_stem(Path(timeline_scene.image_path).stem, position))
        scene_id = f"s{idx:02d}"
        session_scene = session_by_index.get(idx)
        ...
        if session_scene is not None:
            timeline_scene.duration = float(getattr(session_scene, "estimated_duration_sec", timeline_scene.duration) or timeline_scene.duration)
